    ...
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {serviceRes?.map((service: any, index: number) => {
          ...
          return (
            <Link key={service._id} href={`/${service.name.toLowerCase()}/${service._id}`} onClick={() => handleServiceClick(service._id)}>
              <div className={`cursor-pointer border ${bColor} ${bgColor} hover:shadow-lg transition p-4 rounded-lg text-center`}>
